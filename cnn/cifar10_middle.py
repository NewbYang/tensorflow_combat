...
max_steps = 3000
batch_size = 128
data_dir = 'G:\CNN\cifar10'

def variable_with_weight_loss(shape , stddev, wl):
    var = tf.Variable(tf.truncated_normal(shape, stddev=stddev))
    if wl is not True:
        weight_loss = tf.multiply(tf.nn.l2_loss(var),wl,name='weights_loss')
...
for step in range(max_steps):
    ...
    if step % 100 ==0:
        ...
        print("step %d" % step)
        print("example_per_sec: %f" % example_per_sec)
        print("sec_per_batch: %f" % duration)
        num_iter += 1
...
# print('precision @ 1 = %.3f' % predictions)
